...
import { ANALYZE_BASE, ANALYZE_FALLBACK_BASE, fetchJson, formatDate, normalizeLimit, normalizeOffset, parseCsv, requireRows } from './utils.mjs';
...
function mapItem(item, category, timing, rank) {
  const source = item?.data && typeof item.data === 'object' ? { ...item.data, type: item.type ?? item.data.type } : item;
  return {
    rank,
    category: source?.categoryType ?? source?.type ?? category,
    timing,
    sid: valueAt(source, ['sid', 'assetId', 'ticker', 'symbol']),
    date: formatDate(valueAt(source, ['date', 'createdAt', 'publishedAt', 'publishedDate', 'broadcastTime', 'exDate', 'orderDate'])),
    title: valueAt(source, ['title', 'headline', 'subject', 'name']),
    summary: valueAt(source, ['summary', 'description', 'desc']),
    source: valueAt(source, ['source', 'publisher', 'provider']),
    url: valueAt(source, ['url', 'link', 'attachment', 'attachement']),
  };
}
...
export function flattenNewsEventsPayload(payload) {
  const rows = [];
...
cli({
  ...
  func: async (args) => {
    ...
    return requireRows(flattenNewsEventsPayload(payload).slice(0, limit), 'tickertape news-events', 'No Tickertape news/events matched the requested filters');
  },
});
